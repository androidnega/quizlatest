<x-layouts.coordinator>
    <x-slot name="title">{{ __('CSV preview') }}</x-slot>
    <x-slot name="subtitle">{{ __('Validate records before final import') }}</x-slot>

    @php
        $lockedClassroom = $lockedClassroom ?? null;
        $backUrl = $backUrl ?? route('coordinator.students.upload');
    @endphp

    @if ($lockedClassroom)
        <div class="mb-5 rounded-2xl border border-qs-primary/20 bg-qs-soft/50 px-4 py-3 text-sm text-slate-800">
            <p class="font-semibold text-qs-primary">{{ __('Import target') }}</p>
            <p class="mt-1">{{ __('All valid rows will join') }} <span class="font-medium">{{ $lockedClassroom->name }}</span> · {{ $lockedClassroom->program?->name }} · {{ $lockedClassroom->level?->name }}</p>
        </div>
    @endif
    ...
    <div class="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <a href="{{ $backUrl }}" class="qs-btn-secondary inline-flex min-h-[44px] items-center justify-center px-4 text-sm font-semibold">{{ __('Back to upload') }}</a>
