    ...
    <div class="bg-qs-bg rounded-xl shadow-sm p-5">
        <form method="POST" action="{{ route('coordinator.classes.update', $classroom) }}" class="space-y-4">
            ...
            <div>
                ...
                <input id="name" name="name" type="text" value="{{ old('name', $classroom->name) }}" required class="mt-1 block w-full rounded-lg border-qs-soft focus:border-qs-accent focus:ring-qs-accent/40" />
                @error('name')
            ...
            <div>
                ...
                <input
                    ...
                    name="accent_color"
                    type="color"
                    value="{{ old('accent_color', $classroom->accentHex()) }}"
            ...
            <div class="flex items-center gap-2">
                <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $classroom->is_active) ? 'checked' : '' }} class="rounded border-qs-soft text-qs-accent focus:ring-qs-accent/40" />
            ...
            <div class="flex justify-end gap-2 pt-2">
                ...
                <button type="submit" class="qs-btn-primary text-sm">Update Class</button>
            </div>
        </form>
    </div>
</x-layouts.coordinator>
