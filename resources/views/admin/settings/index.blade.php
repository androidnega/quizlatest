<x-layouts.admin :white-workspace="true">
    ...
    @php
        ...
        $fieldLabel = 'mb-1 flex items-center text-sm text-slate-600';
    @endphp

    {{-- Hidden lock/unlock forms — one per lockable key, placed outside the
         main settings form so the inline lock pills can submit them via the
         HTML5 `form` attribute (browsers don't allow nested forms). --}}
    @foreach ($lockable as $lockKey => $lockLabel)
        @php $isLocked = $lockStatesByKey[$lockKey] ?? false; @endphp
        <form
            id="setting-lock-form-{{ $lockKey }}"
            method="post"
            action="{{ $isLocked ? route('admin.settings.unlock') : route('admin.settings.lock') }}"
            class="hidden"
            aria-hidden="true"
        >
            @csrf
            <input type="hidden" name="key" value="{{ $lockKey }}" />
        </form>
    @endforeach

    <div class="rounded-lg border border-slate-200 bg-slate-50/70 px-4 py-3 text-xs text-slate-600">
        <p class="font-semibold text-slate-800">
            <i class="fa-solid fa-lock-open mr-1 text-slate-400" aria-hidden="true"></i>
            <i class="fa-solid fa-lock mr-1 text-emerald-700" aria-hidden="true"></i>
            {{ __('Lock / unlock') }}
        </p>
        <p class="mt-1 leading-relaxed">{{ __('A small lock icon sits next to every setting below. Click it to lock the value (so only super-admins can change it) or unlock it. Locked rows render with a green tint.') }}</p>
    </div>
    ...
    <form method="post" action="{{ route('admin.settings.update') }}" class="space-y-8">
        ...
        <section class="{{ $setPanel }}">
            ...
            <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                ...
                <span>{{ __('Enable WebSockets (Reverb) for live exam session updates') }}</span>
                @include('admin.settings.partials.lock-pill', ['lockKey' => 'enable_live_sockets'])
            </label>
            <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                ...
                <span>{{ __('Allow silent HTTP polling when WebSockets fail or are disabled') }}</span>
                @include('admin.settings.partials.lock-pill', ['lockKey' => 'allow_polling_fallback'])
            </label>
        </section>
        ...
        <section class="{{ $setPanel }}">
            ...
            <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                ...
                <span>Enable OTP for exam start</span>
                @include('admin.settings.partials.lock-pill', ['lockKey' => 'enable_otp'])
            </label>
            <div>
                <label class="{{ $fieldLabel }}">
                    <span>OTP expiry (seconds)</span>
                    @include('admin.settings.partials.lock-pill', ['lockKey' => 'otp_expiry'])
                </label>
            ...
            <div>
                <label class="{{ $fieldLabel }}">
                    <span>OTP verify attempt limit</span>
                    @include('admin.settings.partials.lock-pill', ['lockKey' => 'otp_attempt_limit'])
                </label>
        ...
        <section class="{{ $setPanel }}">
            ...
            <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                ...
                <span>Enable SMS delivery</span>
                @include('admin.settings.partials.lock-pill', ['lockKey' => 'enable_sms'])
            </label>
            <div>
                <label class="{{ $fieldLabel }}">
                    <span>API key</span>
                    @include('admin.settings.partials.lock-pill', ['lockKey' => 'arkesel_api_key'])
                </label>
            ...
            <div>
                <label class="{{ $fieldLabel }}">
                    <span>Sender ID</span>
                    @include('admin.settings.partials.lock-pill', ['lockKey' => 'arkesel_sender_id'])
                </label>
        ...
        <section class="{{ $setPanel }}">
            ...
            <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                ...
                @include('admin.settings.partials.lock-pill', ['lockKey' => 'enable_proctoring'])
            </label>
            <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                ...
                <span>Require exam start verification photo (when proctoring is enabled)</span>
                @include('admin.settings.partials.lock-pill', ['lockKey' => 'require_exam_start_snapshot'])
            </label>
            <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                ...
                <span>Require proctoring camera during the exam (when proctoring is enabled)</span>
                @include('admin.settings.partials.lock-pill', ['lockKey' => 'require_camera_monitoring'])
            </label>
            ...
            <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                ...
                <span>Phone detection enabled</span>
                @include('admin.settings.partials.lock-pill', ['lockKey' => 'phone_detection_enabled'])
            </label>
            <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                ...
                @include('admin.settings.partials.lock-pill', ['lockKey' => 'fullscreen_required'])
            </label>
            <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                ...
                @include('admin.settings.partials.lock-pill', ['lockKey' => 'auto_submit_enabled'])
            </label>
            @if (auth()->user()?->isSuperAdmin())
                ...
                <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                    ...
                    <span>{{ __('Block copy, cut, and paste in the exam answer area') }}</span>
                    @include('admin.settings.partials.lock-pill', ['lockKey' => 'exam_clipboard_lock'])
                </label>
                <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                    ...
                    <span>{{ __('Mitigate common screenshot shortcuts and right-click (when in fullscreen)') }}</span>
                    @include('admin.settings.partials.lock-pill', ['lockKey' => 'exam_screenshot_mitigation'])
                </label>
                <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                    ...
                    <span>{{ __('Try to capture the PrintScreen key via Keyboard Lock when the browser supports it (fullscreen)') }}</span>
                    @include('admin.settings.partials.lock-pill', ['lockKey' => 'exam_screen_record_mitigation'])
                </label>
            @endif
            <div>
                <label class="{{ $fieldLabel }}">
                    <span>Default exam proctoring (JSON, optional)</span>
                    @include('admin.settings.partials.lock-pill', ['lockKey' => 'default_proctoring_settings'])
                </label>
        ...
        <section class="{{ $setPanel }}">
            ...
            <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                ...
                <span>{{ __('Enable AI integrations (master switch)') }}</span>
                @include('admin.settings.partials.lock-pill', ['lockKey' => 'enable_ai'])
            </label>
            <div class="grid gap-4 sm:grid-cols-3">
                <div class="sm:col-span-1">
                    <label class="{{ $fieldLabel }}">
                        <span>{{ __('Provider') }}</span>
                        @include('admin.settings.partials.lock-pill', ['lockKey' => \App\Services\AiIntegrationSettings::CANONICAL_PROVIDER])
                    </label>
                    @php
                ...
                <div class="sm:col-span-2">
                    <label class="{{ $fieldLabel }}">
                        ...
                        @include('admin.settings.partials.lock-pill', ['lockKey' => \App\Services\AiIntegrationSettings::CANONICAL_KEY])
                    </label>
            ...
            <div>
                <label class="{{ $fieldLabel }}">
                    <span>{{ __('Model name') }}</span>
                    @include('admin.settings.partials.lock-pill', ['lockKey' => \App\Services\AiIntegrationSettings::CANONICAL_MODEL])
                </label>
        ...
        @if (auth()->user()?->isSuperAdmin())
            <section class="{{ $setPanel }}">
                ...
                <label class="flex cursor-pointer items-center gap-3 text-sm text-slate-800">
                    ...
                    />
                    <span>{{ __('Use wallet-style mobile dashboard for students') }}</span>
                    @include('admin.settings.partials.lock-pill', ['lockKey' => 'student_dashboard_mobile_wallet'])
                </label>
                ...
                <div class="mt-3 rounded-xl border border-slate-200/80 bg-slate-50/60 p-3 sm:p-4">
                    <p class="flex items-center text-xs font-semibold uppercase tracking-wide text-slate-600">
                        <span>{{ __('Color theme') }}</span>
                        @include('admin.settings.partials.lock-pill', ['lockKey' => \App\Services\StudentDashboardBrandingService::WALLET_THEME_SETTING_KEY])
                    </p>
            ...
            <section class="{{ $setPanel }}">
                <h3 class="flex items-center text-base font-semibold text-slate-900">
                    <span>{{ __('Student exam: presentation mode') }}</span>
                    @include('admin.settings.partials.lock-pill', ['lockKey' => 'student_exam_play_mode'])
                </h3>
                <p class="text-sm text-slate-600">
