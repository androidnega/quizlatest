                        ...
                        <li>
                            <a href="{{ route('student.results.show', $s) }}" class="block px-5 py-4 transition hover:bg-qs-card">
                                <div class="flex flex-wrap items-baseline justify-between gap-2">
                                    ...
                                    <span class="text-xs font-medium uppercase tracking-wide text-qs-muted">{{ $label }}</span>
                                </div>
                                ...
                                @endif
                                <p class="mt-1 text-xs text-qs-muted">{{ $s->end_time?->timezone(config('app.timezone'))->format('Y-m-d H:i') }}</p>
                            </a>
                        </li>
                    @empty
                        <li class="px-5 py-10 text-center text-sm text-qs-muted">{{ __('No submitted exams yet.') }}</li>
                    @endforelse
                </ul>
            </div>
        </div>
