        ...
        <div class="min-h-screen flex bg-white">
            <aside class="hidden md:flex md:w-64 md:flex-col border-r border-[#CFAC81] qs-surface">
                <div class="px-6 py-5 border-b border-[#CFAC81]">
                    <h1 class="text-lg font-semibold qs-heading">QUIZSNAP Admin</h1>
                    <p class="text-xs text-gray-600 mt-1">{{ auth()->user()->name }}</p>
                </div>
                <nav class="flex-1 px-4 py-4 space-y-2">
                    <a href="{{ route('admin.dashboard') }}" class="block px-3 py-2 rounded-md text-sm font-medium {{ request()->routeIs('admin.dashboard') ? 'bg-[#CFAC81] text-white' : 'text-gray-700 hover:bg-white' }}">
                        Dashboard
                    </a>
            ...
            <div class="flex-1">
                <header class="bg-white border-b border-[#CFAC81]">
                    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
                        <div>
                            <h2 class="text-xl font-semibold qs-heading">{{ $title ?? 'Admin Panel' }}</h2>
                            @isset($subtitle)
                        ...
                        <div class="flex items-center gap-4">
                            <a href="{{ route('dashboard') }}" class="text-sm qs-link">Student View</a>
