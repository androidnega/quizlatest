...
Route::any('/admin/{path?}', function (?string $path = null) {
    ...
    if ($path === '' || $path === 'dashboard') {
        return redirect('/dashboard/admin', 301);
    }

    return redirect('/dashboard/admin/'.$path, 301);
})->where('path', '.*');
...
Route::any('/examiner/{path?}', function (?string $path = null) {
    $path = $path === null ? '' : ltrim($path, '/');
    if ($path === '' || $path === 'dashboard') {
        return redirect('/dashboard/examiner', 301);
    }

    return redirect('/dashboard/examiner/'.$path, 301);
})->where('path', '.*');

Route::any('/student/results/{path?}', function (?string $path = null) {
    $path = $path === null ? '' : ltrim($path, '/');
    $base = '/dashboard/student/results';
...
Route::any('/student/practice/{path?}', function (?string $path = null) {
    $path = $path === null ? '' : ltrim($path, '/');
    $base = '/dashboard/student/practice';
...
Route::middleware(['auth', 'verified'])->prefix('dashboard')->group(function () {
    ...
    Route::prefix('examiner')
        ...
        ->middleware('examiner')
        ->group(function () {
            Route::get('/', [ExaminerDashboardController::class, 'index'])->name('dashboard');
